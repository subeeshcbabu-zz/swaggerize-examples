...
/**
 * Test for /repos/{owner}/{repo}/issues/{number}
 */
Test('/repos/{owner}/{repo}/issues/{number}', function (t) {
        ...
        t.test('test  get operation', function (t) {
            Mockgen().requests({
                ...
                request.end(function (err, res) {
                    ...
                    var validate = Validator(api.paths['/repos/{owner}/{repo}/issues/{number}']['get']['responses']['200']['schema']);
        ...
        t.test('test  patch operation', function (t) {
            Mockgen().requests({
                ...
                request.end(function (err, res) {
                    ...
                    t.ok(res.statusCode === 200, 'Ok response status');
                    var Validator = require('is-my-json-valid');
                    var validate = Validator(api.paths['/repos/{owner}/{repo}/issues/{number}']['patch']['responses']['200']['schema']);
                    var response = res.body;
                    if (Object.keys(response).length <= 0) {
                        response = res.text;
                    }
